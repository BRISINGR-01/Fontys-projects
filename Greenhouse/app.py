...
SENSOR_IDS = ["Arduino", "q3bPOMgDza", "Anw2bNFVHb", "rtPGM4n80v", "fFPR9fxgIf", "seZDwgOPZH",
...
def refresh_data():
    ...
    if (board is None):
        data.append([SENSOR_IDS[0], today, 0, current_time(), random.randint(
            13, 35), random.randint(56, 85), random.randint(600, 800)])
    elif (light_measurement is not None):
        data.append([SENSOR_IDS[0], today, 0, current_time(
        ), temperature_measurement, humidity_measurement, light_measurement])

    if (0 not in structuredData[SENSOR_IDS[0]]):
        structuredData[SENSOR_IDS[0]][0] = {}
        structuredData[SENSOR_IDS[0]][0][today] = []
    if (today not in structuredData[SENSOR_IDS[0]][0]):
        structuredData[SENSOR_IDS[0]][0][today] = []

    structuredData[SENSOR_IDS[0]][0][today].append(data[-1])
    write_to_database(data[-1])
...
def refresh_statistics():
    ...
    statisticsData["highest_temperature"] = max(
        statisticsData["highest_temperature"], data[-1][4])
    statisticsData["highest_humidity"] = max(
        statisticsData["highest_humidity"], data[-1][5])
    statisticsData["highest_light"] = max(
        statisticsData["highest_light"], data[-1][6])

    statisticsData["lowest_temperature"] = min(
        statisticsData["lowest_temperature"], data[-1][4])
    statisticsData["lowest_humidity"] = min(
        statisticsData["lowest_humidity"], data[-1][5])
    statisticsData["lowest_light"] = min(
        statisticsData["lowest_light"], data[-1][6])

    dataLength = len(data)
    statisticsData["average_light"] = round(
        ((dataLength - 1) * statisticsData["average_light"] + data[-1][4]) / dataLength)
    statisticsData["average_humidity"] = round(
        ((dataLength - 1) * statisticsData["average_humidity"] + data[-1][5]) / dataLength)
    statisticsData["average_temperature"] = round(
        ((dataLength - 1) * statisticsData["average_temperature"] + data[-1][6]) / dataLength)
...
def init_data():
    ...
    for sensor in SENSOR_IDS:
        ...
        for week in list(range(0, 5)):
            structuredData[sensor][week] = {}
            weekData = list(
                filter(lambda val: int(val[2]) == week, sensorData))

            for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
                structuredData[sensor][week][day] = [
                    val for val in weekData if val[1] == day]
